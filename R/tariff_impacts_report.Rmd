...
# Format monthly dates for CSV: "Dec 2024", "Jan 2025", "Feb", "Mar", ..., "Jan 2026"
# Show year for first entry and every January; abbreviation only otherwise
format_csv_date <- function(dates) {
  # Bar/column charts: text labels as displayed (Datawrapper treats as text)
  m <- format(dates, "%b")
  y <- format(dates, "%Y")
...
format_csv_date_line <- function(dates) {
  # Line charts: standard date format (Datawrapper parses as dates)
  format(dates, "%Y-%m")
}
...
# --- CSV exports (mirrors Excel export section, with streamlined columns) ---

# F1: Daily ETR — CSV only exports plotted columns
if (daily_etr_available && exists("fig1_data_etr")) {
  fig1_csv <- fig1_data_etr %>%
    dplyr::select(Date, `Weighted ETR`)
  write_website_csv(fig1_csv, "F1_daily_etr.csv", "F1")
}

# F2: Effective Tariff Rate — CSV only exports plotted columns
fig2_csv <- fig2_data %>%
  dplyr::select(Date, `Effective tariff rate (%)`)
write_website_csv(fig2_csv, "F2_effective_tariff_rate.csv", "F2")

# F3: Customs Duty Revenue — CSV: formatted dates, plotted columns only
...
# F4: PCE Prices with LP Trend — CSV: no CI, no forecast indicator
...
# T1: Combined IPI Passthrough (June 2025 + latest month)
if (ipi_available && exists("ipi_passthrough_jun") && exists("ipi_passthrough")) {
  # Build combined CSV manually with group headers
...
# F6: Imported PCE Prices — CSV: no CI, no forecast indicator
if (ipi_available && exists("ipi_combined")) {
...
# F8: Import Prices — CSV: no deviation column
...
# F9: Employment Index — CSV: no deviation column
if (tariff_emp_available && exists("trend_recent")) {
...
# F10: Manufacturing Employment Index — CSV: no deviation column
if (tariff_emp_mfg_available && exists("trend_mfg_recent")) {
...
# F11: Industrial Production — CSV: no deviation column
fig11_csv <- ip_trend %>%
  dplyr::select("Date" = date,
                "Industrial production index" = ip_idx,
                "Trend" = trend) %>%
...
# F12: Exchange Rates (daily, keep as-is)
if (exists("fx_daily_indexed")) {
...
# F13: Real Imports and Exports — CSV: no deviation columns
...
# F14: Trade Deviation from Trend — CSV: deviation percentages only
...
# F15: Cumulative Import Gap — CSV: no raw level columns
...
# TA: Combined All-PCE Passthrough (June 2025 + latest month)
{
...
# FA1: Daily ETR by Authority — CSV: no Revision column
if (daily_etr_available && exists("figA1_auth_data")) {
  figA1_csv <- figA1_auth_data %>%
    dplyr::select(-Revision)
  write_website_csv(figA1_csv, "FA1_daily_etr_by_authority.csv", "FA1")
}

# FA2: PCE Prices with Log-Linear Trend — CSV: no CI, no forecast indicator
if (exists("ll_combined")) {
...
html_parts$prices <- paste0(
  ...
  '\n',
  if (ipi_available) fig_placeholder("T1a", "T1_ipi_passthrough.csv") else "",
  if (ipi_available) fig_placeholder("T1b", "T1_ipi_passthrough.csv") else "",
  '\n',
  '<h3>Import Prices</h3>\n',
...
html_parts$trade <- paste0(
  ...
  fig_placeholder("F14", "F14_trade_deviation.csv"),
  fig_placeholder("F15", "F15_cumulative_import_gap.csv"),
  '</section>\n\n'
)
...
html_parts$appendix <- paste0(
  '<section id="appendix">\n',
  '<h2>Appendix</h2>\n',
  if (daily_etr_available) fig_placeholder("FA1", "FA1_daily_etr_by_authority.csv") else "",
  if (ipi_available) fig_placeholder("TA1", "TA_pce_passthrough.csv") else "",
  if (ipi_available) fig_placeholder("TA2", "TA_pce_passthrough.csv") else "",
  ...
  if (exists("ll_combined")) fig_placeholder("FA2", "FA2_pce_prices_loglinear_trend.csv") else "",
  if (exists("ll_deviations")) fig_placeholder("FA3", "FA3_pce_deviation_loglinear.csv") else "",
  '</section>\n\n'
)
