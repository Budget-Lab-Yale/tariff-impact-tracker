...
# Method 2: LP trend passthrough (same parameters as Figure 4)
...
if (ipi_available) {
  ...
  # Method 2: LP trend passthrough (same parameters as Figure 6)
...
# Method 1: Local Projection trend deviations (from Figure 4)
...
# The trend CIs (core_lower/upper, dur_lower/upper in pce_combined) are still used in Figure 4.
...
p1 <- ggplot(trade_deviation, aes(x = date, y = pct_deviation, fill = series)) +
  ...
  labs(
    title = fig_meta("F14a", "title"),
    subtitle = fig_meta("F14a", "subtitle"),
    x = NULL,
    y = fig_meta("F14a", "y_label"),
    fill = NULL
  )
...
p2 <- ggplot(cumulative_trade, aes(x = date, y = cumul_imports)) +
  ...
  geom_vline(xintercept = as.Date("2025-01-01"), linetype = "dashed", color = tbl_gray) +
  labs(
    title = fig_meta("F14b", "title"),
    subtitle = fig_meta("F14b", "subtitle"),
    x = NULL,
    y = fig_meta("F14b", "y_label"),
    caption = fig_caption("F14b")
  )
...
# Figures 6-7: Imported PCE Price Index (conditional)
...
# Figure 9: Tariff Employment Index (conditional)
...
# Figure 10: Manufacturing Employment Index (conditional)
...
# Figure 12: Exchange Rates (Indexed)
...
# Figure 13: Real Imports and Exports (2025 dollars)
...
# Figure 14: Trade Deviation + Cumulative Trade Gap
fig14a_data <- trade_with_trend %>%
...
add_tbl_sheet(wb, "F14a", fig14a_data,
  title    = fig_excel_title("F14a"),
  subtitle = fig_meta("F14a", "subtitle"),
  source   = fig_meta("F14a", "source"))

fig14b_data <- cumulative_imports %>%
  dplyr::select("Date" = date,
                "Real imports" = imports_real,
                "Imports trend" = imports_trend,
                "Monthly gap (billions USD)" = monthly_gap,
                "Cumulative gap (billions USD)" = cumulative_gap)
add_tbl_sheet(wb, "F14b", fig14b_data,
  title    = fig_excel_title("F14b"),
  subtitle = fig_meta("F14b", "subtitle"),
  source   = fig_meta("F14b", "source"))
...
- **Fig1-Fig14**: Raw data for each figure (indexed values, trends, confidence intervals)
...
if (daily_etr_available && exists("fig1_data_etr")) {
  write_website_csv(fig1_data_etr, "F1_daily_etr.csv", "F1")
}

# F2: Effective Tariff Rate
write_website_csv(fig2_data, "F2_effective_tariff_rate.csv", "F2")

# F3: Customs Duty Revenue
write_website_csv(fig3_data_rev, "F3_customs_duty_revenue.csv", "F3")

# F4: PCE Prices with LP Trend
...
# F10: Manufacturing Employment Index
if (tariff_emp_mfg_available && exists("trend_mfg_recent")) {
...
write_website_csv(fig13_trade_data, "F13_real_imports_exports.csv", "F13")

# F14a: Trade Deviation from Trend
write_website_csv(fig14a_data, "F14a_trade_deviation.csv", "F14a")

# F14b: Cumulative Import Gap
write_website_csv(fig14b_data, "F14b_cumulative_import_gap.csv", "F14b")
...
if (daily_etr_available && exists("figA1_auth_data")) {
  write_website_csv(figA1_auth_data, "FA1_daily_etr_by_authority.csv", "FA1")
}
...
html_parts$revenue <- paste0(
  ...
  '\n',
  if (daily_etr_available) fig_placeholder("F1", "F1_daily_etr.csv") else "",
  fig_placeholder("F2", "F2_effective_tariff_rate.csv"),
  fig_placeholder("F3", "F3_customs_duty_revenue.csv"),
  '</section>\n\n'
)
...
html_parts$prices <- paste0(
  ...
  '\n',
  fig_placeholder("F4", "F4_pce_prices_lp_trend.csv"),
  fig_placeholder("F5", "F5_pce_deviation_from_trend.csv"),
  '\n',
  ...
  '\n',
  if (ipi_available) fig_placeholder("F6", "F6_imported_pce_prices.csv") else "",
  if (ipi_available) fig_placeholder("F7", "F7_imported_pce_deviation.csv") else "",
  '\n',
  ...
  '\n',
  fig_placeholder("F8", "F8_import_prices.csv"),
  '</section>\n\n'
)
...
html_parts$labor <- paste0(
  ...
  '\n',
  if (tariff_emp_available) fig_placeholder("F9", "F9_employment_index.csv") else "",
  if (tariff_emp_mfg_available) fig_placeholder("F10", "F10_manufacturing_employment_index.csv") else "",
  '</section>\n\n'
)
...
html_parts$industrial <- paste0(
  ...
  '\n',
  fig_placeholder("F11", "F11_industrial_production.csv"),
  '</section>\n\n'
)
...
html_parts$dollar <- paste0(
  ...
  '\n',
  fig_placeholder("F12", "F12_exchange_rates.csv"),
  '</section>\n\n'
)
...
html_parts$trade <- paste0(
  ...
  '\n',
  fig_placeholder("F13", "F13_real_imports_exports.csv"),
  fig_placeholder("F14a", "F14a_trade_deviation.csv"),
  fig_placeholder("F14b", "F14b_cumulative_import_gap.csv"),
  '</section>\n\n'
)
