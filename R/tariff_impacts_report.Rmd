...
meth_text <- "accompanying Methodology document"
...
# Method 2: LP trend passthrough (same parameters as Figure 3)
...
Versions of Figures 3 and 4 using the log-linear trend approach are included in the [Appendix](#appendix).
...
# Method 1: Local Projection trend deviations (from Figure 3)
...
# The trend CIs (core_lower/upper, dur_lower/upper in pce_combined) are still used in Figure 3.
...
ip_base_val <- get_base_value(industrial_production, "ip_manufacturing", get_base_date(industrial_production, "date"))
...
# Get base date dynamically
ip_base_date <- get_base_date(industrial_production, "date")
dec_base_ip <- get_base_value(industrial_production, "ip_manufacturing", ip_base_date)
...
p1 <- ggplot(trade_deviation, aes(x = date, y = pct_deviation, fill = series)) +
  ...
  labs(
    title = fig_meta("F13a", "title"),
    subtitle = fig_meta("F13a", "subtitle"),
    x = NULL,
    y = fig_meta("F13a", "y_label"),
    fill = NULL
  )
...
p2 <- ggplot(cumulative_trade, aes(x = date, y = cumul_imports)) +
  ...
  geom_vline(xintercept = as.Date("2025-01-01"), linetype = "dashed", color = tbl_gray) +
  labs(
    title = fig_meta("F13b", "title"),
    subtitle = fig_meta("F13b", "subtitle"),
    x = NULL,
    y = fig_meta("F13b", "y_label"),
    caption = fig_caption("F13b")
  )
...
# Figure 3: PCE Prices with Trend
...
# Figure 4: Deviation from Trend
fig4_data <- pce_for_fig4 %>%
  filter(date >= "2024-12-01") %>%
  dplyr::select("Date" = date, 
                "Core goods deviation from trend (pp)" = core_goods_vs_trend, 
                "Durable goods deviation from trend (pp)" = dur_vs_trend)
...
# Figures 5-6: Imported PCE Price Index (conditional)
if (ipi_available && exists("ipi_combined")) {
  ...
  fig6_data <- ipi_for_fig6 %>%
    filter(date >= "2024-12-01") %>%
    dplyr::select("Date" = date,
           "Imported core goods deviation from trend (pp)" = core_goods_vs_trend,
           "Imported durables deviation from trend (pp)" = durables_vs_trend)
...
if (tariff_emp_available && exists("trend_recent")) {
  fig8_data <- trend_recent %>%
    dplyr::select("Date" = date,
                  ...
                  "Trend" = trend,
                  "Deviation from trend (%)" = vs_trend)
  add_tbl_sheet(wb, "F8", fig8_data,
    title    = fig_excel_title("F8"),
    subtitle = fig_meta("F8", "subtitle"),
    source   = fig_meta("F8", "source"))
}

# Figure 9: Manufacturing Employment Index (conditional)
...
# Figure 12: Real Imports and Exports (2025 dollars)
...
add_tbl_sheet(wb, "F12", fig12_data,
  title    = fig_excel_title("F12"),
  subtitle = fig_meta("F12", "subtitle"),
  source   = fig_meta("F12", "source"))

# Figure 13: Trade Deviation + Cumulative Trade Gap
...
add_tbl_sheet(wb, "F13a", fig13_data,
  title    = fig_excel_title("F13a"),
  subtitle = fig_meta("F13a", "subtitle"),
  source   = fig_meta("F13a", "source"))
...
add_tbl_sheet(wb, "F13b", fig13b_data,
  title    = fig_excel_title("F13b"),
  subtitle = fig_meta("F13b", "subtitle"),
  source   = fig_meta("F13b", "source"))
...
if (exists("ll_deviations")) {
  figA2_data <- ll_deviations %>%
    dplyr::select("Date" = date,
                  "Core goods deviation from trend (pp)" = core_goods_vs_trend,
                  "Durable goods deviation from trend (pp)" = durables_vs_trend)
...
- **Fig1-Fig13**: Raw data for each figure (indexed values, trends, confidence intervals)
