...
│   ├── pull_dataweb_naics.R              # USITC DataWeb NAICS-3 pull (replaces manual XLSX)
...
│   ├── customs_duties_by_naics.csv       # Auto-generated by pull_dataweb_naics.R
...
| File | Used By | Source |
|------|---------|--------|
| `customs_duties_by_naics.csv` | Employment index | Auto-generated by `R/pull_dataweb_naics.R` from USITC DataWeb (requires `DATAWEB_API_TOKEN` in `.env`) |
| `BEA - Import Matrix, Before Redefinitions - Summary - 2024.xlsx` | Employment index, Import price index | BEA |
| `BEA - The Use of Commodities by Industry - Summary - 2024.xlsx` | Import price index | BEA |
| `BEA - Commodity-by-Commodity Total Requirements - Summary - 2024.xlsx` | Import price index (optional) | BEA |
| `PCEBridge_Summary.xlsx` | Import price index | BEA |
| `naics_to_bea_crosswalk.csv` | Employment index | Project resource |
| `x_codes.csv` | Employment index | Project resource |
...
- [x] **Automate USITC Customs & Duties download.** Implemented in `R/pull_dataweb_naics.R`. The script POSTs to the USITC DataWeb v2 `runReport` API (`classificationSystem='NAIC'`, NAICS-3 granularity, dual-measure `CONS_CUSTOMS_VALUE` + `CONS_CALC_DUTY`) and writes `input/customs_duties_by_naics.csv`. Authenticates via `DATAWEB_API_TOKEN` in `.env` (looks first in this repo, then in the sibling `tariff-rate-tracker/.env`). Wired into `run_all.R` as Step 3.5; respects `OFFLINE_RUN=TRUE` to skip the live pull and use the cached CSV.
